...
> - 插件任务开始或 Skill 清单、协作结构变化时读取 `../../references/governance-contract.md`，仅按其注册表解析、provider、总览与失败关闭规则动态核对职责。同一任务内清单未变时不重复枚举全部 Skill

> [!note]- **总览同步**
> - 本文件与直接参考是规则权威，Obsidian 生态总览只保留面向人的聚合说明
> - 修改 canonical 后，复核总览中的对应职责段和来源标记，再同步自动区并运行 `../../scripts/skillctl.py audit --json`
> - canonical 哈希漂移、总览说明未复核或 audit 出现 finding 时必须停止，不能把派生说明反向当作规则来源
